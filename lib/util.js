...
function getNvidiaSmi() {
  ...
  if (_windows) {
    try {
      const basePath = process.env.WINDIR + '\\System32\\DriverStore\\FileRepository';
...
function nvidiaSmi(options) {
  return new Promise((resolve) => {
    process.nextTick(() => {
      try {
        exec(getNvidiaSmi() + ' ' + nvidiaSmiOpts, options, function (error, stdout) {
